...
function ManageStudents() {
  ...
  const EditFieldsModal = (props) => {
    ...
    const [studentNameVal, setStudentNameVal] = useState("");

    const handleChange = (e, { value }) => {
      console.log(value);
      setStudentNameVal(value);
    };
    ...
    return (
      ...
      >
        ...
        >
          Update {props.studentName}'s details
        </Modal.Header>
        <Modal.Content>
          <Modal.Description>
  ...
  const DeleteFieldsModal = (props) => {
    return (
      ...
      >
        ...
        <Modal.Content>
          <p>Are you sure you want to delete {props.studentName}</p>
        </Modal.Content>
        <Modal.Actions>
  ...
  const studentList = studentsAPI.map((item) => {
    return (
      <Table.Row key={item.id}>
        <Table.Cell>
          <React.Fragment>{item.studentName}</React.Fragment>
        ...
        <Table.Cell>
          <EditFieldsModal id={item.id} studentName={item.studentName} />
          <DeleteFieldsModal id={item.id} studentName={item.studentName} />
        </Table.Cell>
      </Table.Row>
    );
  });
  ...
  return (
    <>
      <CustomMenu title="Manage Students" />
      <Grid centered>
        <Table unstackable style={{ width: "80%", marginTop: 30 }} celled>
          ...
          <Table.Body>{studentList}</Table.Body>
        </Table>
      </Grid>
    </>
  );
